             ...
             (let* ((cond (cadr e))
                    ...
                    (tests (if or?
                               (begin
                                 (foreach
                                   (lambda (clause)
                                     (let ((gotoifnot (emit `(gotoifnot ,(compile-cond clause break-labels) _))))
                                       (emit end-jump)
                                       (set-car! (cddr gotoifnot) (make&mark-label))))
                                   (butlast (cdr cond)))
                                 (list (emit `(gotoifnot ,(compile-cond (last (cdr cond)) break-labels) _))))
                               (map (lambda (clause)
